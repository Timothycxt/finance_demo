...
corpus = [
    ...
    ['企业负债', '企业欠款', '背负债务', '并购', '市值蒸发', '利益增速下降', '营收下降', '质押股权', '分裂', '拆分子公司', '负债率超标', '负债率踩线', '资金流失', '现金流锐减'],
    # 企业负债
    ['营业收入下降', '交付额下降', '销售额降低', '同比下降', '低于去年', '低于同时期', '公布财报', '负债', '欠款', '负债率', ' 销量减少', '销售额减少', '实现营业收入',
     '营收总额'],  # 营业收入下降
    ...
    ['股权转让', '公司股权转让', '股份转让', '股权拍卖', '股份拍卖', '购并', '流通股购并', '股权购并', '股份购并', '股权合并', '划拨', '非流通股划拨', '非流通股划拨',
     '非流通股划拨']  # 股权转让
]
# 事件类-id映射
...
# 严重程度
dangerous = {0: 3, 1: 3, 2: 3, 3: 3, 4: 30, 5: 5, 6: 5, 7: 5, 8: 2, 9: 2, 10: 2, 11: 7, 12: 7, 13: 7, 14: 10, 15: 30,
             16: 10, 17: 10, 18: 3, 19: 3, 20: 3, 21: 7, 22: 10, 23: 5, 24: 10, 25: 3, 26: 3, 27: 3, 28: 5}
# 事件按年月分类
# 字典 公司id:年份dict
# 年份dict 年份:月份dict
...
def getData(NSRSBM, NSRMC, SSXDM, _year, _month):
    _score = search_score_by_code(str(NSRSBM), NSRMC, SSXDM)
    _score = eval(_score)
    _month_score = 0
    _source = []
    for item in _score:
