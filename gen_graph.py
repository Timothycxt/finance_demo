...
def create_edges(graph):
    ...
    for i, row in data.iterrows():
        ...
        cyber = "MATCH (a:Person {name:'" + row['legal_person'] + "'}), (b:Corporation {name:'" + row['name'] +"'}) MERGE (a)-[:legal_person]->(b)"
        graph.run(cyber)
        # link industry to corporation
        cyber = "MATCH (a:Industry {name:'" + row['industry'] + "'}), (b:Corporation {name:'" + row['name'] +"'}) MERGE (a)-[:industry]->(b)"
        graph.run(cyber)
        # link type to corporation
        cyber = "MATCH (a:Type {name:'" + row['type'] + "'}), (b:Corporation {name:'" + row['name'] +"'}) MERGE (a)-[:type]->(b)"
        graph.run(cyber)
        # link admin_div to corporation
        cyber = "MATCH (a:Admin_div {name:'" + row['admin_div'] + "'}), (b:Corporation {name:'" + row['name'] +"'}) MERGE (a)-[:admin_div]->(b)"
        graph.run(cyber)

        for name in row['member'].split(','):
            name = name.strip()
            # link member to corporation
            cyber = "MATCH (a:Person {name:'" + name + "'}), (b:Corporation {name:'" + row['name'] +"'}) MERGE (a)-[:member]->(b)"
            graph.run(cyber)
